...
router.get("/posts/:postId", async (req, res) => {
    ...
    Number(postId);
    const detailpost = await Post.findOne({ order: [['createdAt', 'DESC']], where: { postId } });
...
router.put("/posts/:postId", authMiddleware, async (req, res) => {
    ...
    if (detailpost) {
        ...
        return res.status(400).json({ success: false, errorMessage: "해당 글이 존재하지 않습니다." });
    }
...
router.post("/posts/like/:postId", authMiddleware, async (req, res) => {
    ...
    if (detailpost) {
        if (!likedpost) {
            ...
            return res.status(400).json({ success: false, errorMessage: "이미 좋아요를 눌렀습니다." });
        }
    }
    else {
        return res.status(400).json({ success: false, errorMessage: "해당 글이 존재하지 않습니다." });
